...
triangular_number_sequence = [(t - 1) * t // 2 for t in range(minutes + 1)]
...
class Blueprint(object):
    ...
    def get_optimal_plan(self, ore: int, clay: int, obsidian: int, geode: int, or_robot: int, cl_robot: int,
                         ob_robot: int, ge_robot: int, remaining_minutes: int = minutes, depth=0) -> int:
        # stash: ore, clay, obsidian, geode
        # robots: ore, clay, obsidian, geode
        max_value = 0

        if depth <= minutes - 10 and (ore, clay, obsidian, geode, or_robot, cl_robot, ob_robot, ge_robot, remaining_minutes) in self.cache:
            # print('cache hit')
...
def solution():
    total = 1
    for i in range(3):
        plan = blueprints[i].get_optimal_plan(0, 0, 0, 0, 1, 0, 0, 0)
        blueprints[i].cache = {}
        total *= plan
        print(total)
